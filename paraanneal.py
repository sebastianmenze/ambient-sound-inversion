# -*- coding: utf-8 -*-
"""
parallel parameter optimization python code (simulated annealing):
...
def received_pressure(p_vector,tl_db):
    ...
    for i_receiver in range(tl_db.shape[1]):  
        ...
        for i_source in range(tl_db.shape[0]):      
            ...
            single_source_RL= 20.0*np.log10(p_vector[i_source]) + tl_db[i_source,i_receiver] 
            if single_source_RL < 0:
                 received_db.append(0) 
            else:
                received_db.append(single_source_RL)        
        #print(received_db)
        p_received.append( sum(10.0**(np.array(received_db)/np.array(20))) )
    #print(p_received)
    db_received =20.0*np.log10(np.array(p_received))      
    return db_received
